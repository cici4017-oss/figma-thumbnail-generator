...
/**
 * 표준 "썸네일 자동화 요청서" Excel 워크북(01_작업요청/02_상품목록/03_채널목록/04_작성가이드)을
 * 만드는 순수 빌더. 상품/채널 데이터를 하드코딩하지 않고 항상 호출부가 넘기는 products/channels를
 * source of truth로 쓴다 — packages/core/scripts/generateWorkOrderTemplate.mjs(저장소에 커밋되는
 * 정적 템플릿 재생성용)와 figma-plugin의 "Excel 양식 다운로드" 버튼(플러그인 안에서 즉시 생성,
 * 회사 PC에 파일을 따로 반입할 필요 없음) 둘 다 이 함수 하나를 호출한다 — 상품/채널 하드코딩
 * 목록을 두 번 유지하지 않기 위함이다.
 *
 * 이 파일은 core/import 아래에 있다 — exceljs를 실제로 쓰는 소비자만 명시적으로
 * '@thumbnail-generator/core/import'에서 가져오게 해서, code.ts처럼 Excel을 다루지 않는
 * 소비자의 번들에 exceljs가 섞여 들어가지 않게 하기 위함이다(xlsxReader.ts와 동일한 원칙).
 */
...
const PRODUCT_GROUP_LABELS: Record<ProductRegistryEntry['productGroup'], string> = {
  'simple-meal': '간편식',
  'baby-food': '영유아',
};
...
/**
 * 병합 셀을 쓰지 않고 경고 배너처럼 보이게 한다: 첫 컬럼에만 문구를 쓰고, 같은 행의 나머지
 * 컬럼들은 배경색만 맞춰서 자연스러운 텍스트 오버플로우로 배너처럼 보이게 한다.
 */
function addLockedSheetBanner(worksheet: ExcelJS.Worksheet, colCount: number, message: string) {
...
export interface WorkOrderTemplateDeps {
  products: ProductRegistryEntry[];
  channels: Channel[];
}

export function buildWorkOrderTemplateWorkbook(deps: WorkOrderTemplateDeps): ExcelJS.Workbook {
  ...
  const simpleMealProducts = deps.products.filter((p) => p.productGroup === 'simple-meal');
  const babyProducts = deps.products.filter((p) => p.productGroup === 'baby-food');

  let r = 3;
  const simpleMealStartRow = r;
  for (const p of simpleMealProducts) {
    productSheet.getRow(r).values = [p.code, PRODUCT_GROUP_LABELS['simple-meal'], p.brand, p.name, p.capacity ?? ''];
    r++;
  }
  ...
  for (const p of babyProducts) {
    productSheet.getRow(r).values = [p.code, PRODUCT_GROUP_LABELS['baby-food'], p.brand, p.name, p.capacity ?? ''];
    r++;
  }
  const babyEndRow = r - 1;

  // 드롭다운/자동입력 모두 "상품명"(D열) 기준이므로 defined name도 D열을 가리킨다.
  workbook.definedNames.add(`'02_상품목록'!$D$${simpleMealStartRow}:$D$${simpleMealEndRow}`, 'PRODUCTS_간편식');
  ...
  deps.channels.forEach((c, i) => {
    channelSheet.getRow(3 + i).values = [c.label, c.id, ''];
  });
  const channelStartRow = 3;
  const channelEndRow = 2 + deps.channels.length;
  ...
  workHeaderRow.values = ['작업ID', '순번', '상품군', '채널', '상품명', '상품코드(자동입력)', '수량', '딱지여부', '비고'];
  ...
  const productCodeFormula = (row: number) =>
  ...
  const exampleRows: (string | number | undefined)[][] = [
  ...
  const guideLines: [string, string][] = [
    ...
    [
      '참고',
      '이 템플릿은 packages/core/scripts/generateWorkOrderTemplate.mjs 스크립트(저장소 관리자용) 또는 플러그인의 "Excel 양식 다운로드" 버튼(사용자용)으로 언제든 다시 받을 수 있습니다 — 둘 다 최신 상품/채널 데이터를 그대로 반영합니다.',
    ],
  ];
